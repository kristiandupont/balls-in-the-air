...
function* BallCircle(
  ...
  this.schedule(async (g: SVGGElement) => {
    ...
    circles.forEach((circle, i) => {
      circle.style.transition =
        "r 300ms cubic-bezier(0.68, -0.55, 0.265, 1.55)";
      if (i === 0) {
        // Outer border circle
  ...
  for ({ ball, isSelected, onRegisterCrescent } of this) {
    ...
    // Initial offset at 45 degrees (will be updated imperatively in tick)
    const offsetDistance = radius * 0.03;
    const initialAngle = 45 * (Math.PI / 180);
...
function* TaskSimulation(
  ...
  balls.forEach((ball) => {
    ...
    // Initialize crescent angle tracking
    ...
    balls.forEach((ball) => {
      ...
      if (crescentData && crescentEl) {
        ...
        if (distance > 0.1) {
          ...
          const transitionSpeed = Math.min(distance * 0.02, 0.3);
